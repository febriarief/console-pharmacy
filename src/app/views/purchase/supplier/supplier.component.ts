...
import { ModalService, SupplierService } from 'src/app/services'
...
export class SupplierComponent implements OnInit
{
	...
	constructor(
        ...
        this.modulePermission = 'Purchase.Supplier.'
        ...
        this.formTitle = 'Tambah Data Supplier'
        this.viewMode = false
        this.editMode = false
	...
	loadData(page: any = 1): void {
		...
		this._supplierService.get(this.filter)
		.subscribe({
			next: (res: any) => {
    ...
    editData(entry: any): void {
        this.formTitle = 'Ubah Data Supplier'
        this.viewMode = false
        this.editMode = true
        this.form = { ...entry }
    ...
    viewData(entry: any): void {
        this.formTitle = 'Lihat Data Supplier'
        this.viewMode = true
        this.editMode = false
        this.form = { ...entry }
	...
	clearFormValidation(): void {
		this.formValidation = {
            ...
            address: { valid: true, message: null },
    ...
    save(): void {
        ...
        if (this.editMode) {
            this._supplierService.update(this.form, this.form.id)
            .subscribe({
                next: () => {
                    this.isSubmitting = false
                    this.closeForm()
            ...
            this._supplierService.store(this.form)
            .subscribe({
                next: () => {
                    this.isSubmitting = false
                    this.closeForm()
