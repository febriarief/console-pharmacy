...
import { ModalService, UnitService } from 'src/app/services'
...
export class UnitComponent implements OnInit
{
	...
	constructor(
        ...
        this.modulePermission = 'MasterItem.Unit.'
        ...
        this.formTitle = 'Tambah Data Satuan'
        this.editMode = false
	...
	loadData(page: any = 1): void {
		...
		this._unitService.get(this.filter)
		.subscribe({
			next: (res: any) => {
    ...
    editData(entry: any): void {
        this.formTitle = 'Ubah Data Satuan'
        this.editMode = true
        this.form = { ...entry }
	...
	validate(): boolean {
		...
		if (!this.form.short) {
            this.formValidation.short.valid = false
            this.formValidation.short.message = 'Kolom isian singkatan tidak boleh kosong'
            valid = false
		}
    ...
    save(): void {
        ...
        if (this.editMode) {
            this._unitService.update(this.form, this.form.id)
            .subscribe({
                next: () => {
                    this.isSubmitting = false
                    this.closeForm()
            ...
            this._unitService.store(this.form)
            .subscribe({
                next: () => {
                    this.isSubmitting = false
                    this.closeForm()
